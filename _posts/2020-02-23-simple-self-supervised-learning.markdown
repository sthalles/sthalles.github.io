...
For quite some time now, we know about the benefits of transfer learning in Computer Vision (CV) applications. Nowadays, pre-trained Deep Convolution Neural Networks (DCNNs) are the first go-to pre-solution to learn a new task. These large models are trained on huge supervised corpora, like the ImageNet. And most importantly, their features are known to adapt well to new problems. These benefits are particularly interesting when annotated training data is scarce. In situations like this, we take the models' pre-trained weights, append a new classifier layer on top of it, and retrain the network. This is called transfer learning and is one of the most used techniques in CV. Aside from a few tricks when doing this process, it has been shown (many times) that the final model tends to be better if compared with random initialization.
...
Note that the same is applied to both augmented version of a given observation. This way, the number of negative pairs is increased even more.
...
In fact, in the original implementation, SimCLR is trained with batch sizes as large as 8192. By following the ideas described above, this batch size produces 16382 negative examples per positive pair. In addition, the authors also showed that larger batches (hence more negatives) tend to produce better results.
...
To train the model, I used the [STL-10 dataset](http://ai.stanford.edu/~acoates/stl10/). STL-10 contains 10 different classes with a reasonable small number of observations per class. Most importantly, it contains a larger unsupervised set with 100000 unlabeled images -- *that is the bulk of images used for training*.
...
After training, we need a way to evaluate the learned representations. One way to check the quality of these features is by using a ***linear evaluation protocol.***

The idea is to train linear classifiers on fixed representations from the SimCLR network. To do that, we take the training data, pass it through the pre-trained SimCLR model, and store the output representations. Note that at this point, we do not need the projection head $g(.)$ anymore.
